...
export const enhancedVisualVerificationTools = [
  ...
  // Simple visual verification tools for AI agents
  {
    name: 'quick_workflow_check',
    description: 'Fast workflow error detection - optimized for AI agents. Quickly scans for "could not find property option" and other common errors.',
    inputSchema: {
      type: 'object',
      properties: {
        workflowId: {
  ...
  {
    name: 'detect_workflow_errors',
    description: 'Comprehensive visual error detection with browser automation. Logs into n8n, navigates to workflow, and detects errors visually.',
    inputSchema: {
      type: 'object',
      properties: {
        workflowId: {
          type: 'string',
          description: 'Workflow ID to verify'
        },
        waitTime: {
          type: 'number',
          description: 'Seconds to wait for errors to appear',
          default: 3
        }
      },
      required: ['workflowId']
  ...
  },
  {
    ...
    description: 'Take screenshot of workflow using browser automation. Useful for visual confirmation of workflow state.',
    inputSchema: {
      type: 'object',
      properties: {
        workflowId: {
          type: 'string',
          description: 'Workflow ID to screenshot'
        },
        fullPage: {
          type: 'boolean',
  ...
  },
  {
    ...
    description: 'Find workflows by name pattern using n8n API. Fast search without browser automation.',
    inputSchema: {
      type: 'object',
      properties: {
        namePattern: {
          type: 'string',
          description: 'Text to search for in workflow names'
        }
      },
...
export async function executeEnhancedVisualTool(name: string, args: any): Promise<any> {
  switch (name) {
    ...
    case 'quick_workflow_check':
      return handleQuickWorkflowCheck(args);
    case 'detect_workflow_errors':
      return handleDetectWorkflowErrors(args);
    case 'take_workflow_screenshot':
      return handleTakeWorkflowScreenshot(args);
    case 'find_workflows_by_name':
      return handleFindWorkflowsByName(args);
    default:
      throw new Error(`Unknown enhanced visual verification tool: ${name}`);
  }
}
...
/**
 * Simple visual verification handlers for AI agents
 */

async function handleQuickWorkflowCheck(args: any): Promise<any> {
  const { workflowId } = args;
  
  if (!visualSystem) {
    throw new Error('Visual system not initialized. Call setup_enhanced_visual_verification first.');
  }
  
  try {
    // Use the enhanced visual system for issue detection
    const issues = await visualSystem.detectEnhancedVisualIssues(workflowId);
    const hasErrors = issues.length > 0;
    
    return {
      workflowId,
      status: hasErrors ? 'ERROR_DETECTED' : 'NO_ERRORS',
      hasPropertyError: issues.some(issue => issue.recommendation.includes('property')),
      errorContext: issues.map(issue => issue.recommendation).join('; '),
      timestamp: new Date().toISOString(),
      message: hasErrors ? 
        `🚨 Quick check detected ${issues.length} errors in workflow ${workflowId}` : 
        `✅ No errors detected in workflow ${workflowId}`
    };
  } catch (error) {
    throw new Error(`Quick workflow check failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}

async function handleDetectWorkflowErrors(args: any): Promise<any> {
  const { workflowId, waitTime = 3 } = args;
  ...
  try {
    // Use comprehensive analysis with browser automation
    const analysis = await visualSystem.analyzeWorkflowComprehensively(workflowId);
    
    // Count all issues from different categories
    const allIssues = [
      ...analysis.dataFlowIssues,
      ...analysis.performanceRisks,
      ...analysis.bestPracticeViolations,
    ...
    return {
      workflowId,
      verification: {
        canvasFound: analysis.executionReadiness.ready,
        pageLoadable: analysis.executionReadiness.ready,
        waitTime
      },
      errors: allIssues,
      analysis: {
      ...
      message: allIssues.length > 0 ? 
        `🎯 Comprehensive scan found ${allIssues.length} issues in workflow ${workflowId}` : 
        `✅ No issues detected in workflow ${workflowId}`
    };
  } catch (error) {
    throw new Error(`Comprehensive error detection failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}

async function handleTakeWorkflowScreenshot(args: any): Promise<any> {
  const { workflowId, fullPage = true } = args;
  ...
  try {
    // Use comprehensive analysis to get screenshot data
    const analysis = await visualSystem.analyzeWorkflowComprehensively(workflowId);
    ...
    return {
      ...
      timestamp: new Date().toISOString(),
      message: `📸 Screenshot analysis completed for workflow ${workflowId}`
    };
  } catch (error) {
...
async function handleFindWorkflowsByName(args: any): Promise<any> {
  const { namePattern } = args;
  
  if (!apiClient) {
    throw new Error('API client not initialized. Call setup_enhanced_visual_verification first.');
  }
  
  try {
    const workflows = await apiClient.listWorkflows();
    const matches = workflows.data.filter((workflow: any) => 
      workflow.name && workflow.name.toLowerCase().includes(namePattern.toLowerCase())
    );
    ...
    return {
      ...
      message: matches.length === 0 ? 
        `❌ No workflows found matching "${namePattern}"` : 
        `✅ Found ${matches.length} workflows matching "${namePattern}"`
    };
  } catch (error) {
    throw new Error(`Workflow search failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}