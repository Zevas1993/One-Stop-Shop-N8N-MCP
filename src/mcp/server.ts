  ...
  private async executeToolInternal(name: string, args: any): Promise<any> {
    switch (name) {
      ...
      case 'get_node_summary':
        return this.getNodeSummary(args);
      case 'check_compatibility':
        return this.checkCompatibility(args);
      case 'validate_before_adding':
        return this.validateBeforeAdding(args);
...
`,
  ...
  private async validateWorkflowUnified(args: any): Promise<any> {
    ...
    switch (mode) {
      case 'quick':
        ...
        validationOptions.quickMode = true; // Flag for minimal response
        break;
      case 'full':
  ...
  private async findTemplatesUnified(args: any): Promise<any> {
    const { mode = 'keywords', nodeTypes, query, task, limit = 20, detail = 'summary' } = args;
    ...
    switch (mode) {
      case 'nodes':
        ...
        const nodeResult = this.listNodeTemplates(nodeTypes, limit);
        return detail === 'summary' ? this.summarizeTemplateResult(nodeResult) : nodeResult;
      case 'keywords':
        if (!query) {
          throw new Error('query is required for keywords mode');
        }
        const keywordResult = this.searchTemplates(query, limit);
        return detail === 'summary' ? this.summarizeTemplateResult(keywordResult) : keywordResult;
      case 'task':
        if (!task) {
          throw new Error('task is required for task mode');
        }
        const taskResult = this.getTemplatesForTask(task);
        return detail === 'summary' ? this.summarizeTemplateResult(taskResult) : taskResult;
      case 'all':
        // For all mode, use search with empty query to get all templates
        const allResult = this.searchTemplates('', limit);
        return detail === 'summary' ? this.summarizeTemplateResult(allResult) : allResult;
      default:
  ...
  private async summarizeTemplateResult(resultPromise: Promise<any>): Promise<any> {
    const result = await resultPromise;
    
    // If result has templates array, summarize each template
  ...
  // Progressive Disclosure Tools Implementation
  private async getNodeSummary(args: any): Promise<any> {
    const { nodeType } = args;
    
    try {
      // Use existing getNodeInfoUnified but with essentials mode and strip down to summary
      const fullInfo = await this.getNodeInfoUnified({ nodeType, detail: 'essentials' });
      
      // Return ultra-light summary - just key information
      return {
        nodeType: fullInfo.nodeType,
        displayName: fullInfo.displayName,
        description: fullInfo.description,
        category: fullInfo.metadata?.category || 'unknown',
        keyProperties: fullInfo.requiredProperties?.slice(0, 3) || [], // Only first 3 required props
      ...
      throw new Error(`Failed to get node summary: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  private async checkCompatibility(args: any): Promise<any> {
    const { sourceNode, targetNode } = args;
    
    try {
      // Get basic info for both nodes
      ...
      const isCompatible = this.performCompatibilityCheck(sourceInfo, targetInfo);
      ...
      throw new Error(`Failed to check compatibility: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }

  private performCompatibilityCheck(source: any, target: any): { result: boolean, reason: string, confidence: string } {
    // Basic compatibility rules
    
    // Trigger nodes should come first
    if (source.category === 'trigger' && target.category === 'trigger') {
      return { result: false, reason: 'Cannot connect two trigger nodes', confidence: 'high' };
    }
    
    // All nodes generally accept JSON input from other nodes
    if (source.category !== 'unknown' && target.category !== 'unknown') {
      return { result: true, reason: 'Standard JSON data flow compatibility', confidence: 'medium' };
    }
    
    return { result: true, reason: 'Basic compatibility assumed', confidence: 'low' };
  }

  private async validateBeforeAdding(args: any): Promise<any> {
    ...
    try {
      ...
      if (targetPosition === 'start' && existingNodes.some((n: string) => n.includes('trigger'))) {
        validations.push({
          level: 'error',
      ...
      const similarNodes = existingNodes.filter((existing: string) => {
        return existing.split('.')[1] === nodeType.split('.')[1]; // Same base name
      });
      ...
      return {
        ...
        validations,
        summary: isValid ? 'Node can be safely added' : 'Issues found - review before adding',
        responseSize: '<2KB'
      };
      
    } catch (error) {
      throw new Error(`Failed to validate node placement: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
