...
export class EnhancedDocumentationFetcher {
   ...
   */
  async getEnhancedNodeDocumentation(
    nodeType: string
  ): Promise<EnhancedNodeDocumentation | null> {
    if (!this.cloned) {
  ...
  private extractOperations(markdown: string): OperationInfo[] {
    ...
    const operationsMatch = markdown.match(
      /##\s+Operations\s*\n([\s\S]*?)(?=\n##|\n#|$)/i
    );
  ...
  private extractApiMethods(markdown: string): ApiMethodMapping[] {
    ...
    const tableRegex =
      /\|.*Resource.*\|.*Operation.*\|.*(?:Slack API method|API method|Method).*\|[\s\S]*?\n(?=\n[^|]|$)/gi;
  ...
  private extractCodeExamples(markdown: string): CodeExample[] {
    ...
    while ((match = codeBlockRegex.exec(markdown)) !== null) {
      ...
      const beforeText = markdown.substring(
        Math.max(0, beforeCodeIndex - 200),
        beforeCodeIndex
      );
  ...
  private extractTemplates(markdown: string): TemplateInfo[] {
    ...
    const templateWidgetMatch = markdown.match(
      /\[\[\s*templatesWidget\s*\(\s*[^,]+,\s*'([^']+)'\s*\)\s*\]\]/
    );
  ...
  private extractRelatedResources(markdown: string): RelatedResource[] {
    ...
    const relatedMatch = markdown.match(
      /##\s+(?:Related resources|Related|Resources)\s*\n([\s\S]*?)(?=\n##|\n#|$)/i
    );
  ...
  private extractRequiredScopes(markdown: string): string[] {
    ...
    const scopesMatch = markdown.match(
      /##\s+(?:Required scopes|Scopes)\s*\n([\s\S]*?)(?=\n##|\n#|$)/i
    );
