...
function mapErrorToJsonRpc(error: any): { code: number; message: string } {
  ...
  if (/invalid param|REQUIRED|INVALID/.test(msg))
...
export async function startFixedHTTPServer() {
  ...
  const mcpServer = await createUnifiedMCPServer();
  ...
  app.post(
    ...
    async (req: express.Request, res: express.Response): Promise<void> => {
      ...
      const maxBodyBytes =
        (parseInt(process.env.MCP_HTTP_MAX_BODY_KB || "512") || 512) * 1024; // default 512KB
        ...
        req.on("end", async () => {
          try {
            ...
            switch (jsonRpcRequest.method) {
              ...
              case "tools/list":
                // Use the dynamic tool list from the server
