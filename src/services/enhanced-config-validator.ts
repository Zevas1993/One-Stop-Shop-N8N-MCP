...
export class EnhancedConfigValidator extends ConfigValidator {
    ...
    switch (mode) {
      case "minimal":
        ...
        return properties.filter(
          (prop) => prop.required && this.isPropertyVisible(prop, config)
        );
  ...
  private static isPropertyRelevantToOperation(
    ...
    if (prop.displayOptions?.show) {
      ...
      if (operation.resource && show.resource) {
        const expectedResources = Array.isArray(show.resource)
          ? show.resource
          : [show.resource];
      ...
      if (operation.action && show.action) {
        const expectedActions = Array.isArray(show.action)
          ? show.action
          : [show.action];
  ...
  private static enhanceSlackValidation(
    ...
    if (resource === "message" && operation === "send") {
      ...
      if (!config.channel && !config.channelId) {
        const channelError = result.errors.find(
          (e) => e.property === "channel" || e.property === "channelId"
        );
        if (channelError) {
          channelError.message =
            'To send a Slack message, specify either a channel name (e.g., "#general") or channel ID';
          channelError.fix =
            'Add channel: "#general" or use a channel ID like "C1234567890"';
        }
      }
    ...
    if (operation === "append") {
      ...
      if (config.range && !config.range.includes("!")) {
        result.warnings.push({
          ...
          suggestion:
            'Format: "SheetName!A1:B10" or "SheetName!A:B" for entire columns',
        });
      }
    }
  }
  ...
  private static generateNextSteps(result: EnhancedValidationResult): string[] {
    ...
    if (requiredErrors.length > 0) {
      steps.push(
        `Add required fields: ${requiredErrors
          .map((e) => e.property)
          .join(", ")}`
      );
    }
    ...
    if (typeErrors.length > 0) {
      steps.push(
        `Fix type mismatches: ${typeErrors
          .map((e) => `${e.property} should be ${e.fix}`)
          .join(", ")}`
      );
    }
    ...
    if (valueErrors.length > 0) {
      steps.push(
        `Correct invalid values: ${valueErrors
          .map((e) => e.property)
          .join(", ")}`
      );
    }
    ...
    for (const error of errors) {
      ...
      if (!existing) {
        ...
        const existingLength =
          (existing.message?.length || 0) + (existing.fix?.length || 0);
        const newLength =
          (error.message?.length || 0) + (error.fix?.length || 0);
