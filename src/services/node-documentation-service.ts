...
CREATE TABLE IF NOT EXISTS extraction_stats (
  ...
  async storeNode(nodeInfo: NodeInfo): Promise<void> {
    ...
    logger.info(
      `[NodeDocumentationService] Storing node: ${nodeInfo.nodeType}`,
    ...
    stmt.run({
      ...
      operations: nodeInfo.operations
        ? JSON.stringify(nodeInfo.operations)
        : null,
      apiMethods: nodeInfo.apiMethods
        ? JSON.stringify(nodeInfo.apiMethods)
        : null,
      documentationExamples: nodeInfo.documentationExamples
        ? JSON.stringify(nodeInfo.documentationExamples)
        : null,
      templates: nodeInfo.templates ? JSON.stringify(nodeInfo.templates) : null,
      relatedResources: nodeInfo.relatedResources
        ? JSON.stringify(nodeInfo.relatedResources)
        : null,
      requiredScopes: nodeInfo.requiredScopes
        ? JSON.stringify(nodeInfo.requiredScopes)
        : null,
      exampleWorkflow: nodeInfo.exampleWorkflow
        ? JSON.stringify(nodeInfo.exampleWorkflow)
        : null,
      exampleParameters: nodeInfo.exampleParameters
        ? JSON.stringify(nodeInfo.exampleParameters)
        : null,
      propertiesSchema: nodeInfo.propertiesSchema
        ? JSON.stringify(nodeInfo.propertiesSchema)
        : null,
  ...
  async rebuildDatabase(): Promise<{
    ...
    await this.ensureInitialized();
    console.error(
      "[NodeDocumentationService] Starting complete database rebuild..."
    );
    ...
    try {
      ...
      console.error(
        `[NodeDocumentationService] Found ${stats.total} nodes to process`
      );
      ...
      for (let i = 0; i < availableNodes.length; i += batchSize) {
        ...
        await Promise.all(
          batch.map(async (node) => {
            try {
              ...
              console.error(
                `[NodeDocumentationService] Processing node: ${nodeType}`
              );

              // Extract source code
              const nodeData = await this.extractor.extractNodeSource(
                nodeType,
                node.location
              );
              ...
              const nodeDefinition = this.parseNodeDefinition(
                nodeData.sourceCode
              );

              // Get enhanced documentation
              const enhancedDocs =
                await this.docsFetcher.getEnhancedNodeDocumentation(nodeType);

              // Generate example
              const example =
                ExampleGenerator.generateFromNodeDefinition(nodeDefinition);
              ...
              const nodeInfo: NodeInfo = {
                ...
                name: node.name,
                displayName:
                  nodeDefinition.displayName || node.displayName || node.name,
                description:
              ...
              stats.successful++;
              console.error(
                `[NodeDocumentationService] Successfully processed: ${nodeType}`
              );
              logger.debug(`Processed node: ${nodeType}`);
            } catch (error) {
              console.error(
                `[NodeDocumentationService] Failed to process ${node.name}:`,
                error
              );
              stats.failed++;
              const errorMsg = `Failed to process ${node.name}: ${
        ...
        logger.info(
          `Progress: ${Math.min(i + batchSize, availableNodes.length)}/${
            stats.total
          } nodes processed`
        );
      }
      ...
      logger.info(
        `Database rebuild complete: ${stats.successful} successful, ${stats.failed} failed`
      );
    } catch (error) {
  ...
  private parseNodeDefinition(sourceCode: string): any {
    ...
    try {
      ...
      // Display name
      const displayNameMatch = sourceCode.match(
        /displayName\s*[:=]\s*['"`]([^'"`]+)['"`]/
      );
      if (displayNameMatch) {
      ...
      // Category/group
      const groupMatch = sourceCode.match(
        /group\s*[:=]\s*\[['"`]([^'"`]+)['"`]\]/
      );
      if (groupMatch) {
      ...
      // Subtitle
      const subtitleMatch = sourceCode.match(
        /subtitle\s*[:=]\s*['"`]([^'"`]+)['"`]/
      );
      if (subtitleMatch) {
      ...
      if (
        sourceCode.includes("implements.*ITrigger") ||
  ...
  private rowToNodeInfo(row: any): NodeInfo {
    return {
      ...
      documentationExamples: row.documentation_examples
        ? JSON.parse(row.documentation_examples)
        : null,
      ...
      requiredScopes: row.required_scopes
        ...
        : null,
      exampleWorkflow: row.example_workflow
        ? JSON.parse(row.example_workflow)
        : null,
      exampleParameters: row.example_parameters
        ? JSON.parse(row.example_parameters)
        : null,
