...
export class NodeParser {
  ...
  private extractNodeType(description: any, packageName: string): string {
    ...
    if (!name) {
      // Fallback: try to generate a name from displayName or class name
      if (description.displayName) {
        name = description.displayName.replace(/\s+/g, "");
        // lowercase first letter to match n8n convention
        name = name.charAt(0).toLowerCase() + name.slice(1);
      } else {
        // Last resort: use a placeholder that will be flagged
        name = "unknownNode";
      }
    }
  ...
  private extractVersion(nodeClass: any): string {
      ...
      if (instance?.description?.version) {
        ...
        if (Array.isArray(version)) {
          ...
          const maxVersion = Math.max(
            ...version.map((v: any) => parseFloat(v.toString()))
          );
    ...
    if (description?.version) {
      if (Array.isArray(description.version)) {
        const maxVersion = Math.max(
          ...description.version.map((v: any) => parseFloat(v.toString()))
        );
        return maxVersion.toString();
      } else if (
  ...
  private detectVersioned(nodeClass: any): boolean {
    ...
    try {
      ...
      if (
        ...
        Array.isArray(instance.description.version)
      ) {
        return true;
      }
    } catch (e) {
  ...
  private extractCategoryFromPath(path: string): string {
    ...
    if (parts.includes("nodes")) {
      ...
      if (
        nodesIndex + 1 < parts.length &&
        parts[nodesIndex + 1] !== parts[parts.length - 1]
      ) {
