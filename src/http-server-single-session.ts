...
export class SingleSessionHTTPServer {
  ...
  constructor() {
    ...
    const generationModel =
      process.env.GENERATION_MODEL || "Qwen/Qwen3-4B-Instruct";
  ...
  private async resetSession(): Promise<void> {
    ...
    try {
      ...
      logger.info("Created new single session successfully", {
        sessionId: this.session.sessionId,
      });
    } catch (error) {
  ...
  async start(): Promise<void> {
    ...
    app.get("/api/patterns", (req, res) => {
      res.json({
        patterns: [
          {
            id: "data-fetch",
            name: "Data Fetch Pattern",
            description: "Fetch data from external sources",
          },
          {
            id: "data-transform",
            name: "Data Transform Pattern",
    ...
    app.post(
      ...
      async (req: express.Request, res: express.Response): Promise<void> => {
        try {
          const { executionId, workflowId, userId, workflow, feedback } =
            req.body;
          ...
          const updateDecision =
            await this.learningService.processWorkflowFeedback(
              learningFeedback
            );
    ...
    this.expressServer = app.listen(port, host, () => {
      ...
      console.log(
        `   POST /api/orchestrate  - Generate workflows via natural language`
      );
